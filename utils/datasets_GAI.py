...
from utils.general import (LOGGER, NUM_THREADS, check_dataset, check_requirements, check_yaml, clean_str, segment2box,
                           xyn2xy, xywh2xyxy, xywhn2xyxy, xyxy2xywhn, resample_segments)
...
def create_dataloader_rgb_ir(path1, path2, imgsz, batch_size, stride, names, single_cls, hyp=None, augment=False,
...
class LoadMultiModalImagesAndLabels(Dataset):  # for training/testing
    ...
    def __init__(self, path_rgb, path_ir, cls_names, img_size=640, batch_size=16, augment=False, hyp=None, rect=False,
                 image_weights=False,
        ...
        cache_rgb_path = (p_rgb if p_rgb.is_file() else Path(self.label_files_rgb[0]).parent).with_suffix(
            '.cache')  # cached labels
        try:
            cache_rgb, exists_rgb = np.load(cache_rgb_path, allow_pickle=True).item(), True  # load dict
            assert cache_rgb['version'] == self.cache_version  # same version
            assert cache_rgb['hash'] == get_hash(self.label_files_rgb + self.img_files_rgb)  # same hash
        except:
        ...
        cache_ir_path = (p_ir if p_ir.is_file() else Path(self.label_files_ir[0]).parent).with_suffix(
            '.cache')  # cached labels
        try:
            cache_ir, exists_ir = np.load(cache_ir_path, allow_pickle=True).item(), True  # load dict
            assert cache_ir['version'] == self.cache_version  # same version
            assert cache_ir['hash'] == get_hash(self.label_files_ir + self.img_files_ir)  # same hash
        except:
        ...
        self.labels_rgb = list(labels_rgb)  # labels(list[array]): n_imgs * array(num_gt_perimg, [cls_id, poly])
        self.shapes_rgb = np.array(shapes_rgb, dtype=np.float64)  # img_ori shape
        ...
        self.labels_ir = list(labels_ir)  # labels(list[array]): n_imgs * array(num_gt_perimg, [cls_id, poly])
        self.shapes_ir = np.array(shapes_ir, dtype=np.float64)  # img_ori shape
    ...
    def cache_labels(self, imgfiles, labelfiles, path=Path('./labels.cache'), prefix=''):
        ...
        with Pool(NUM_THREADS) as pool:
            pbar = tqdm(pool.imap(verify_image_label,
                                  zip(img_files, label_files, repeat(prefix), repeat(self.cls_names))),
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            # Load image
            img_rgb, img_ir, (h0, w0), (h, w), labels_rgb, labels_ir = load_image_rgb_ir(self, index)
            ...
            labels_rgb_new = self.labels_rgb[index].copy()
            if labels_rgb.size:  # normalized xywh to pixel xyxy format
                # labels[:, 1:] = xywhn2xyxy(labels[:, 1:], ratio[0] * w, ratio[1] * h, padw=pad[0], padh=pad[1])
                labels_rgb_new[:, [1, 3, 5, 7]] = labels_rgb[:, [1, 3, 5, 7]] * ratio[0] + pad[0]
                labels_rgb_new[:, [2, 4, 6, 8]] = labels_rgb[:, [2, 4, 6, 8]] * ratio[1] + pad[1]
                labels_rgb = labels_rgb_new
            labels_ir_new = self.labels_ir[index].copy()
            if labels_ir.size:  # normalized xywh to pixel xyxy format
                # labels[:, 1:] = xywhn2xyxy(labels[:, 1:], ratio[0] * w, ratio[1] * h, padw=pad[0], padh=pad[1])
                labels_ir_new[:, [1, 3, 5, 7]] = labels_ir[:, [1, 3, 5, 7]] * ratio[0] + pad[0]
                labels_ir_new[:, [2, 4, 6, 8]] = labels_ir[:, [2, 4, 6, 8]] * ratio[1] + pad[1]
                labels_ir = labels_ir_new
            if self.augment:
                img_rgb, img_ir, labels_rgb, labels_ir = random_perspective_rgb_ir(img_rgb, img_ir, labels_rgb, labels_ir,
                                                                                   degrees=self.hyp['degrees'],
                                                                                   translate=self.hyp['translate'],
                                                                                   scale=self.hyp['scale'],
                                                                                   shear=self.hyp['shear'],
                                                                                   perspective=self.hyp[
                                                                                       'perspective'])  # border to remove
        ...
        if self.augment:
            ...
            if random.random() < hyp['flipud']:
                ...
                if nl_ir:
                    # labels[:, 2] = 1 - labels[:, 2]
                    labels_ir[:, 2::2] = img_h - labels_ir[:, 2::2] - 1
            # Flip left-right
            if random.random() < hyp['fliplr']:
                ...
                if nl_ir:
                    # labels[:, 1] = 1 - labels[:, 1]
                    labels_ir[:, 1::2] = img_w - labels_ir[:, 1::2] - 1
        if nl_rgb:
            ...
            labels_obb_rgb = np.concatenate((labels_rgb[:, :1], rboxes, csl_labels), axis=1)
            labels_mask_rgb = (rboxes[:, 0] >= 0) & (rboxes[:, 0] < img_rgb.shape[1]) \
                          & (rboxes[:, 1] >= 0) & (rboxes[:, 0] < img_rgb.shape[0]) \
                          & (rboxes[:, 2] > 5) | (rboxes[:, 3] > 5)
            labels_obb_rgb = labels_obb_rgb[labels_mask_rgb]
            nl_rgb = len(labels_obb_rgb)
        if hyp:
            c_num = 7 + hyp['cls_theta']  # [index_of_batch clsid cx cy l s theta gaussian_θ_labels]
        else:
            c_num = 187
        labels_out_rgb = torch.zeros((nl_rgb, c_num))
        if nl_rgb:
            # labels_out[:, 1:] = torch.from_numpy(labels)
            labels_out_rgb[:, 1:] = torch.from_numpy(labels_obb_rgb)
        if nl_ir:
            # *[clsid poly] to *[clsid cx cy l s theta gaussian_θ_labels] θ∈[-pi/2, pi/2) non-normalized
            rboxes, csl_labels = poly2rbox(polys=labels_ir[:, 1:],
                                           num_cls_thata=hyp['cls_theta'] if hyp else 180,
                                           radius=hyp['csl_radius'] if hyp else 6.0,
                                           use_pi=True, use_gaussian=True)
            labels_obb_ir = np.concatenate((labels_ir[:, :1], rboxes, csl_labels), axis=1)
            labels_mask_ir = (rboxes[:, 0] >= 0) & (rboxes[:, 0] < img_ir.shape[1]) \
                          & (rboxes[:, 1] >= 0) & (rboxes[:, 0] < img_ir.shape[0]) \
                          & (rboxes[:, 2] > 5) | (rboxes[:, 3] > 5)
            labels_obb_ir = labels_obb_ir[labels_mask_ir]
            nl_ir = len(labels_obb_ir)
        if hyp:
            c_num = 7 + hyp['cls_theta']  # [index_of_batch clsid cx cy l s theta gaussian_θ_labels]
        else:
            c_num = 187
        labels_out_ir = torch.zeros((nl_ir, c_num))
        if nl_ir:
            # labels_out[:, 1:] = torch.from_numpy(labels)
            labels_out_ir[:, 1:] = torch.from_numpy(labels_obb_ir)
        # Convert
        ...
        return torch.from_numpy(img_all), labels_out_rgb,labels_out_ir, self.img_files_rgb[index], shapes
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self, path, cls_names, img_size=640, batch_size=16, augment=False, hyp=None, rect=False,
        ...
        if self.rect:
            ...
            self.batch_shapes = np.ceil(np.array(shapes) * img_size / stride + pad).astype(
                np.int) * stride  # (nb, [h_rect, w_rect])
    ...
    @staticmethod
    def collate_fn(batch):
        img, label_rgb, labels_ir, path, shapes = zip(*batch)  # transposed
        for i_rgb, l_rgb in enumerate(label_rgb):
            l_rgb[:, 0] = i_rgb  # add target image index for build_targets()
        for i_ir, l_ir in enumerate(labels_ir):
            l_ir[:, 0] = i_ir  # add target image index for build_targets()
        return torch.stack(img, 0), torch.cat(label_rgb, 0), torch.cat(labels_ir, 0), path, shapes
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        # place img in img4
        if i == 0:  # top left
            img4 = np.full((s * 2, s * 2, img.shape[2]), 255, dtype=np.uint8)  # base image with 4 tiles
...
def load_image_rgb_ir(self, index):
    ...
    # img_rgb = None
    # img_ir = None
    label_rgb = self.labels_rgb[index].copy()  # labels (array): (num_gt_perimg, [cls_id, poly])
    label_ir = self.labels_ir[index].copy()
    if (img_rgb is None) and (img_ir is None):  # not cached
        ...
        if r != 1:  # if sizes are not equal
            ...
            img_ir = cv2.resize(img_ir, (int(w0 * r), int(h0 * r)),
                                interpolation=cv2.INTER_AREA if r < 1 and not self.augment else cv2.INTER_LINEAR)
            label_ir[:, 1:] *= r
        return img_rgb, img_ir, (h0, w0), img_rgb.shape[:2], label_rgb, label_ir  # img, hw_original, hw_resized
    else:
        return self.imgs_rgb[index], self.imgs_ir[index], self.img_hw0_rgb[index], self.img_hw_rgb[index], \
               self.labels_rgb[index], label_ir[index]  # img, hw_original, hw_resized
...
def load_mosaic_RGB_IR(self, index1, index2):
    ...
    for i, index in enumerate(indices):
        ...
        img_rgb, img_ir, _, (h, w), img_label_rgb, img_label_ir = load_image_rgb_ir(self, index)

        if i == 0:  # top left
            img4_rgb = np.full((s * 2, s * 2, img_rgb.shape[2]), 255, dtype=np.uint8)  # base image with 4 tiles
            img4_ir = np.full((s * 2, s * 2, img_ir.shape[2]), 255, dtype=np.uint8)  # base image with 4 tiles
            x1a, y1a, x2a, y2a = max(xc - w, 0), max(yc - h, 0), xc, yc  # xmin, ymin, xmax, ymax (large image)
            x1b, y1b, x2b, y2b = w - (x2a - x1a), h - (y2a - y1a), w, h  # xmin, ymin, xmax, ymax (small image)
        elif i == 1:  # top right
            x1a, y1a, x2a, y2a = xc, max(yc - h, 0), min(xc + w, s * 2), yc
        ...
        labels_rgb, segments_rgb = img_label_rgb.copy(), self.segments_rgb[index].copy()
        labels_ir, segments_ir = img_label_ir.copy(), self.segments_ir[index].copy()
        if labels_rgb.size:
            labels_rgb[:, [1, 3, 5, 7]] = img_label_rgb[:, [1, 3, 5, 7]] + padw
            labels_rgb[:, [2, 4, 6, 8]] = img_label_rgb[:, [2, 4, 6, 8]] + padh
            labels_ir[:, [1, 3, 5, 7]] = img_label_ir[:, [1, 3, 5, 7]] + padw
            labels_ir[:, [2, 4, 6, 8]] = img_label_ir[:, [2, 4, 6, 8]] + padh
    ...
    h_filter = 2 * s
    w_filter = 2 * s
    labels_mask_ir = poly_filter(polys=labels4_ir[:, 1:].copy(), h=h_filter, w=w_filter)
    labels4_ir = labels4_ir[labels_mask_ir]
...
def random_perspective_rgb_ir(img_rgb, img_ir, targets_rgb=(), targets_ir=(), segments_rgb=(), segments_ir=(),
    ...
    img = img_rgb
    targets_rgb = targets_rgb
    targets_ir = targets_ir
    ...
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
        if perspective:
            ...
            img_rgb = cv2.warpPerspective(img_rgb, M, dsize=(width, height), borderValue=(255, 255, 255))
            img_ir = cv2.warpPerspective(img_ir, M, dsize=(width, height), borderValue=(255, 255, 255))
        else:  # affine
            # img = cv2.warpAffine(img, M[:2], dsize=(width, height), borderValue=(114, 114, 114))
            img_rgb = cv2.warpAffine(img_rgb, M[:2], dsize=(width, height), borderValue=(255, 255, 255))
            img_ir = cv2.warpAffine(img_ir, M[:2], dsize=(width, height), borderValue=(255, 255, 255))
    ...
    n_rgb = len(targets_rgb)
    if n_rgb:
        use_segments = any(x.any() for x in segments)
        new = np.zeros((n_rgb, 4))
        if use_segments:  # warp segments
            segments = resample_segments(segments)  # upsample
            for i, segment in enumerate(segments):
                xy_rgb = np.ones((len(segment), 3))
                xy_rgb[:, :2] = segment
                xy_rgb = xy_rgb @ M.T  # transform
                xy_rgb = xy_rgb[:, :2] / xy_rgb[:, 2:3] if perspective else xy_rgb[:, :2]  # perspective rescale or affine

                # clip
                new[i] = segment2box(xy_rgb, width, height)

        else:  # warp boxes
            xy_rgb = np.ones((n_rgb * 4, 3))
            # xy[:, :2] = targets[:, [1, 2, 3, 4, 1, 4, 3, 2]].reshape(n * 4, 2)  # x1y1, x2y2, x1y2, x2y1
            xy_rgb[:, :2] = targets_rgb[:, 1:].reshape(n_rgb * 4, 2)  # x1y1, x2y2, x1y2, x2y1
            xy_rgb = xy_rgb @ M.T  # transform
            xy_rgb = (xy_rgb[:, :2] / xy_rgb[:, 2:3] if perspective else xy_rgb[:, :2]).reshape(n_rgb, 8)  # perspective rescale or affine
        targets_mask_rgb = poly_filter(polys=xy_rgb, h=height, w=width)
        targets_rgb[:, 1:] = xy_rgb
        targets_rgb = targets_rgb[targets_mask_rgb]
    n_ir = len(targets_ir)
    if n_ir:
        use_segments = any(x.any() for x in segments)
        new = np.zeros((n_ir, 4))
        if use_segments:  # warp segments
            segments = resample_segments(segments)  # upsample
            for i, segment in enumerate(segments):
                xy_ir = np.ones((len(segment), 3))
                xy_ir[:, :2] = segment
                xy_ir = xy_ir @ M.T  # transform
                xy_ir = xy_ir[:, :2] / xy_ir[:, 2:3] if perspective else xy_ir[:, :2]  # perspective rescale or affine

                # clip
                new[i] = segment2box(xy_ir, width, height)
        else:  # warp boxes
            xy_ir = np.ones((n_ir * 4, 3))
            # xy[:, :2] = targets[:, [1, 2, 3, 4, 1, 4, 3, 2]].reshape(n * 4, 2)  # x1y1, x2y2, x1y2, x2y1
            xy_ir[:, :2] = targets_ir[:, 1:].reshape(n_ir * 4, 2)  # x1y1, x2y2, x1y2, x2y1
            xy_ir = xy_ir @ M.T  # transform
            xy_ir = (xy_ir[:, :2] / xy_ir[:, 2:3] if perspective else xy_ir[:, :2]).reshape(n_ir,8)  # perspective rescale or affine
        ...
        # targets = targets[i]
        # targets[:, 1:5] = new[i]
        targets_mask_ir = poly_filter(polys=xy_ir, h=height, w=width)
        targets_ir[:, 1:] = xy_ir
        targets_ir = targets_ir[targets_mask_ir]
    return img_rgb, img_ir, targets_rgb, targets_ir
...
def load_mosaic9(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        # place img in img9
        if i == 0:  # center
            img9 = np.full((s * 3, s * 3, img.shape[2]), 255, dtype=np.uint8)  # base image with 4 tiles
            h0, w0 = h, w
...
def verify_image_label(args):
    ...
    if isinstance(cls_name_list, dict):
        class_names = list(cls_name_list.values())
    else:
...
def dataset_stats(path='coco128.yaml', autodownload=False, verbose=False, profile=False, hub=False):
    ...
    for split in 'train_ir', 'val_ir', 'test_ir', 'train_rgb', 'val_rgb', 'test_rgb':
