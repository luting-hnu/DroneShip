...
def check_dataset(data, autodownload=True):
    ...
    for k in 'train_ir', 'val_ir', 'test_ir', 'train_rgb','val_rgb','test_rgb':
    ...
    # train_ir, val_ir, test_ir,train_rgb,val_rgb,test_rgb, s = (data.get(x) for x in ('train_ir', 'val_ir', 'test_ir', 'train_rgb','val_rgb','test_rgb', 'download'))
    # if val_ir :
    #     val = [Path(x).resolve() for x in (val_ir if isinstance(val_ir, list) else [val_ir])]  # val path
...
def non_max_suppression_obb(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        rboxes[:, :2] = rboxes[:, :2] - c  # rboxes (offset by class)
    return output
def non_max_suppression_obb_ill(prediction_rgb,prediction_ir,w_rgb,w_ir, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                        labels=(), max_det=1500):
    """Runs Non-Maximum Suppression (NMS) on inference results_obb
    Args:
        prediction (tensor): (b, n_all_anchors, [cx cy l s obj num_cls theta_cls])
    ...
    Returns:
        list of detections, len=batch_size, on (n,7) tensor per image [xylsθ, conf, cls] θ ∈ [-pi/2, pi/2)
    """
    # prediction_rgb[..., 4] = prediction_rgb[..., 4] * w_rgb
    # prediction_ir[..., 4] = prediction_ir[..., 4] * w_ir
    prediction = torch.concat((prediction_rgb,prediction_ir),dim = 1)
    nc = prediction.shape[2] - 5 - 180  # number of classes
    xc = prediction[..., 4] > conf_thres  # candidates
    class_index = nc + 5
