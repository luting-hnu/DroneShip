...
class Annotator:
    ...
    def convert_image_to_annotation_path(self,image_path):
        # Replace the source directory and file extension
        base_path = '/home/server08/ljc/yolov5_obb-master/'
        image_subpath = 'dataset/data_ship/images/test/ir/'
        annotation_subpath = 'runs/detect/ship/labels/'
        annotation_extension = '.txt'

        # Get the filename without extension
        filename = os.path.splitext(os.path.basename(image_path))[0]

        # Construct the new annotation path
        annotation_path = os.path.join(base_path, annotation_subpath, filename + annotation_extension)
    ...
    def poly_label(self, poly, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        # # 将 polygon_list 和 label 转换为字符串格式
        #     source = self.convert_image_to_annotation_path(source)
        #     polygon_str = ' '.join([f'{x},{y}' for x, y in polygon_list])
        #     result_str = f'{polygon_str.replace(","," ")} {label}'
        #
        #     # 将结果字符串追加写入文本文件，并换行
        #     with open(source, 'a') as file:  # 'a' 模式表示追加
...
def enhance_contrast(img, method='normalize'):
    if method == 'normalize':
...
def feature_visualization(x, module_type, stage, n=32, save_dir=Path('runs/detect/exp')):
    """
    可视化模型某层的前 n 个特征图通道，并保存为 png 和 npy。
    """
    if 'Detect' not in module_type:
        batch, channels, height, width = x.shape
        if height > 1 and width > 1:
            if channels < n:
                print(f"⚠️  通道数只有 {channels}，小于指定的 {n}，将仅可视化前 {channels} 个通道。")
            n = min(n, channels)
            ...
            print(f"✅ 已保存特征图到: {f}")
def visualize_single_channel(x, module_type, stage, channel_idx=27, save_dir=Path('runs/detect/exp/single')):
    """
    可视化并保存模型某层某个通道的特征图。
    参数：
        x: 特征图 Tensor，shape 为 [B, C, H, W]
        module_type: 模块类型（用于命名）
        stage: 当前层的编号
        channel_idx: 要保存的通道索引（默认第0通道）
        save_dir: 保存目录
    """
    if 'Detect' not in module_type:
        b, c, h, w = x.shape
        if h > 1 and w > 1:
            if channel_idx >= c:
                print(f"❌ 通道索引 {channel_idx} 超出范围，最大值为 {c-1}")
                return

            # 提取通道特征图
            fmap = x[0, channel_idx].cpu().detach().numpy()
            fmap -= fmap.min()
            fmap /= (fmap.max() + 1e-6)

            # 保存图像
            img_path = save_dir / f"stage{stage}_{module_type.split('.')[-1]}_channel{channel_idx}.png"
            plt.imsave(img_path, fmap, cmap='viridis')

            # 保存原始 numpy 数据
            npy_path = img_path.with_suffix('.npy')
            np.save(npy_path, fmap)

            print(f"✅ 已保存特征图通道 {channel_idx} 到: {img_path}")
def feature_visualization_all(x, module_type, stage, save_dir=Path('runs/detect/exp')):
    """
    x:              Features to be visualized
    module_type:    Module type
